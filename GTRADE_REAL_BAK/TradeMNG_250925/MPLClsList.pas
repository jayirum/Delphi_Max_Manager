...
//    else sUserTp := '';
    ...
    begin
      if cbUserGrade.ItemIndex <> 0 then sUserTp := Format(' AND A1.USER_GRADE = %s ', [IntToStr(cbUserGrade.ItemIndex)])
      else if chUserTp.Checked then sUserTp := Format(' AND A1.USER_GRADE IN (%s,%s) ', [QuotedStr('2'),QuotedStr('7')])
      else sUserTp := '';
    end
    else
    begin
      if chUserTp.Checked then sUserTp := ' AND A1.USER_GRADE = 2 '
      else sUserTp := ' AND A1.USER_GRADE <> 7 ';
    end;
