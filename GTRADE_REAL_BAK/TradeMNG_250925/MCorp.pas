...
type
  TfmCorp = class(TfmBasic)
    ...
    dbMainCNTR_F_RT: TIntegerField;
    dbMainCNTR_N_RT: TIntegerField;
    dbMainCNTR_GF_RT: TIntegerField;
    dbMainNOWPRC_CNTR: TStringField;
    dbMainREMN_INIT_HOGA: TIntegerField;
    dbMainMIT_MKT_YN: TStringField;
    dbMainNULLHOGA_YN: TStringField;
    dbMainFIX_MITSL: TStringField;
    dbMainIN_BANKNM: TStringField;
    dbMainSL_CANCEL: TStringField;
    dbMainPARTNER_DISP_DAY: TIntegerField;
...
procedure TfmCorp.dbMainAfterPost(DataSet: TDataSet);
var
	sSql: String;
begin
  inherited;
	...
	with DataSet do
  begin
  	sSql := Format('UPDATE BANK_MST SET BANK_NM = %s, BANK_ACNT_NO=%s, BANK_ACNT_NM=%s WHERE BANK_CD=1',
                      [QuotedStr(FieldByName('BANK_NM').AsString),
                       QuotedStr(FieldByName('BANK_ACNT_NO').AsString),
                       QuotedStr(FieldByName('BANK_ACNT_NM').AsString)]);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          sSql := Format('UPDATE ACNT_MST SET MINAMT_GUJA = %s', [FieldByName('GUJA_MIN_AMT_1').NewValue]);
          Uni_Open(MastDb.dbExec, sSql);
        end
        else
        begin
