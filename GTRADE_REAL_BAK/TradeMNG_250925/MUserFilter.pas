...
begin
  ...
  if edUserID.Text <> '' then
  	sFilter := sFilter + Format('A.USER_ID LIKE %s AND ',  [QuotedStr('%' + edUserID.Text + '%')]);
  if edUserNm.Text <> '' then
  	sFilter := sFilter + Format('A.USER_NM LIKE %s AND ', [QuotedStr('%' + edUserNm.Text + '%')]);
  if edPartId.Text <> '' then
    sFilter := sFilter + Format('A.PARTNER_ID LIKE %s AND ',  [QuotedStr('%' + edPartId.Text + '%')]);
  if edPartNick.Text <> '' then
    sFilter := sFilter + Format('A.PARTNER_NICK_NM LIKE %s AND ',  [QuotedStr('%' + edPartNick.Text + '%')]);
  if cbUserGrade.ItemIndex <> -1 then
    sFilter := sFilter + Format('A.USER_GRADE LIKE %s AND ', [QuotedStr('%' + cbUserGrade.Values[cbUserGrade.ItemIndex] + '%')]);
  if cbUserPart.ItemIndex <> -1 then
    sFilter := sFilter + Format('A.PART_CD LIKE %s AND ', [QuotedStr('%' + cbUserPart.Values[cbUserPart.ItemIndex] + '%')]);
...
//    sFilter := sFilter + Format('ACNT_STAT LIKE %s AND ', [QuotedStr('%' + cbAcntStat.Values[cbAcntStat.ItemIndex] + '%')]);
  if cbServerIp.ItemIndex <> -1 then
    sFilter := sFilter + Format('A.SERVER_IP LIKE %s AND ', [QuotedStr('%' + cbServerIp.Values[cbServerIp.ItemIndex] + '%')]);

  sFilter := sFilter + 'NOT A.USER_ID IS NULL';
