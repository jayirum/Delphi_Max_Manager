...
type
  TfmNego = class(TfmBasic)
    ...
    dbMainCLA_CAL2: TStringField;
    dbMainNGA_CAL: TStringField;
    dbMainGCA_CAL: TStringField;
    dbMainESA_CAL1: TStringField;
    dbMainADA_CAL: TStringField;
    dbMainBPA_CAL: TStringField;
    dbMainCLA2: TFloatField;
    dbMainNGA: TFloatField;
    dbMainGCA: TFloatField;
    dbMainESA: TFloatField;
    dbMainADA: TFloatField;
    dbMainBPA: TFloatField;
    dbMainJYA: TFloatField;
    dbMainJYA2: TFloatField;
    dbMainJYA3: TFloatField;
    dbMainJYA4: TFloatField;
    dbMainJYA_CAL: TStringField;
    dbMainJYA_CAL2: TStringField;
    dbMainJYA_CAL3: TStringField;
    dbMainJYA_CAL4: TStringField;
...
begin
  ...
  begin
    ...
    if FieldByName('NGA').AsString = '' then FieldByName('NGA_CAL').AsString := '미적용'
    else FieldByName('NGA_CAL').AsString := FormatFloat(__FORMAT_AMT, FieldByName('NGA').AsFloat);

    if FieldByName('GCA').AsString = '' then FieldByName('GCA_CAL').AsString := '미적용'
    else FieldByName('GCA_CAL').AsString := FormatFloat(__FORMAT_AMT, FieldByName('GCA').AsFloat);

    if FieldByName('ESA').AsString = '' then FieldByName('ESA_CAL').AsString := '미적용'
    else FieldByName('ESA_CAL').AsString := FormatFloat(__FORMAT_AMT, FieldByName('ESA').AsFloat);

    if FieldByName('ADA').AsString = '' then FieldByName('ADA_CAL').AsString := '미적용'
    else FieldByName('ADA_CAL').AsString := FormatFloat(__FORMAT_AMT, FieldByName('ADA').AsFloat);

    if FieldByName('BPA').AsString = '' then FieldByName('BPA_CAL').AsString := '미적용'
    else FieldByName('BPA_CAL').AsString := FormatFloat(__FORMAT_AMT, FieldByName('BPA').AsFloat);

    if FieldByName('JYA').AsString = '' then FieldByName('JYA_CAL').AsString := '미적용'
    else FieldByName('JYA_CAL').AsString := FormatFloat(__FORMAT_AMT, FieldByName('JYA').AsFloat);

    if FieldByName('NQA').AsString = '' then FieldByName('NQA_CAL').AsString := '미적용'
    else FieldByName('NQA_CAL').AsString := FormatFloat(__FORMAT_AMT, FieldByName('NQA').AsFloat);

    if FieldByName('YMA').AsString = '' then FieldByName('YMA_CAL').AsString := '미적용'
    else FieldByName('YMA_CAL').AsString := FormatFloat(__FORMAT_AMT, FieldByName('YMA').AsFloat);

    if FieldByName('SCNA').AsString = '' then FieldByName('SCNA_CAL').AsString := '미적용'
    else FieldByName('SCNA_CAL').AsString := FormatFloat(__FORMAT_AMT, FieldByName('SCNA').AsFloat);

    if FieldByName('HSIA').AsString = '' then FieldByName('HSIA_CAL').AsString := '미적용'
    else FieldByName('HSIA_CAL').AsString := FormatFloat(__FORMAT_AMT, FieldByName('HSIA').AsFloat);
  end;
end;
...
begin
  sQuery := ' SELECT 																																																					  ' +
            ...
            '       (SELECT NEGO_AMTRT FROM NEGO_CMSN WHERE ACNT_NO = A.ACNT_NO AND (ARTC_CD = %s OR ARTC_CD = %s)) AS EA,                      	' +
            '       (SELECT NEGO_AMTRT FROM NEGO_CMSN WHERE ACNT_NO = A.ACNT_NO AND ARTC_CD = %s) AS CLA,                      	' +
            '       (SELECT NEGO_AMTRT FROM NEGO_CMSN WHERE ACNT_NO = A.ACNT_NO AND ARTC_CD = %s) AS NGA,                      	' +
            '       (SELECT NEGO_AMTRT FROM NEGO_CMSN WHERE ACNT_NO = A.ACNT_NO AND ARTC_CD = %s) AS GCA,                      	' +
            '       (SELECT NEGO_AMTRT FROM NEGO_CMSN WHERE ACNT_NO = A.ACNT_NO AND ARTC_CD = %s) AS ESA,                      	' +
            '       (SELECT NEGO_AMTRT FROM NEGO_CMSN WHERE ACNT_NO = A.ACNT_NO AND (ARTC_CD = %s OR ARTC_CD = %s)) AS ADA,                      	' +
            '       (SELECT NEGO_AMTRT FROM NEGO_CMSN WHERE ACNT_NO = A.ACNT_NO AND (ARTC_CD = %s OR ARTC_CD = %s)) AS BPA,                      	' +
            '       (SELECT NEGO_AMTRT FROM NEGO_CMSN WHERE ACNT_NO = A.ACNT_NO AND (ARTC_CD = %s OR ARTC_CD = %s)) AS JYA,                      	' +
            '       (SELECT NEGO_AMTRT FROM NEGO_CMSN WHERE ACNT_NO = A.ACNT_NO AND ARTC_CD = %s) AS NQA,                      	' +
            '       (SELECT NEGO_AMTRT FROM NEGO_CMSN WHERE ACNT_NO = A.ACNT_NO AND ARTC_CD = %s) AS YMA,                      	' +
            '       (SELECT NEGO_AMTRT FROM NEGO_CMSN WHERE ACNT_NO = A.ACNT_NO AND ARTC_CD = %s) AS SCNA,                     	' +
            '       (SELECT NEGO_AMTRT FROM NEGO_CMSN WHERE ACNT_NO = A.ACNT_NO AND ARTC_CD = %s) AS HSIA,                     	' +
            '       B.ACNT_TP AS ACNTTP                                                                                         ' +
            ' FROM NEGO_CMSN A,                                                                                                 ' +
            '      ACNT_MST B                                                                                                   ' +
