...
//  if edFind.Text <> '' then sNm := Format(' AND USER_NM = %s', [QuotedStr(edFind.Text)]);
  ...
  if edFind.Text <> '' then sNm := Format(' AND (USER_NM LIKE %s OR USER_ID LIKE %s) ', [QuotedStr('%'+edFind.Text+'%'), QuotedStr('%'+edFind.Text+'%')]);
...
begin
  ...
  begin
    if FieldByName('IO_TP').AsString = '입금' then Canvas.Font.Color := clRed
...
begin
  try
    ...
    sSql := Format( 'SELECT  USER_ID, '+
                    'RQST_TM, '+
                    '(SELECT CODE_VALUE_NM FROM CODE_MST WHERE CODE_ID = %s AND CODE_VALUE = A.IO_TP) AS IO_TP, '+
                    'ACNT_TP, '+
                    'ACNT_NO, '+
                    'USER_NM, '+
                    'RQST_AMT, '+
                    '(SELECT CODE_VALUE_NM FROM CODE_MST WHERE CODE_ID = %s AND CODE_VALUE = A.RSLT_TP) AS RSLT_TP, '+
                    'RSLT_AMT, '+
                    'RSLT_MNG_ID, '+
                    'RQST_TRADE_DT, '+
                    'RQST_SYS_DT, '+
