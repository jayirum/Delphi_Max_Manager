...
uses
  ...
  ThdTimer, ODBCUniProvider, AccessUniProvider;
...
type
  ...
  TMastDB = class(TDataModule)
    ...
    tmRetry: TThreadedTimer;
    AccessUniProvider: TAccessUniProvider;
    ...
    procedure tmRetryTimer(Sender: TObject);
  private
...
var
  ...
  __MINI_YN: Boolean;                                                           // 미니여부

  __PresentName: Boolean;                                                       // 현황판 종목명표시

  __BankCD_Default: Integer;                                                    // 은행 기본값
  __OrdParter_Visible: String;                                                  // 메인 주문체결화면 파트너 Enabled

  __OverNGT_Default: String;                                                    // 오버나잇 기본값

  __PWD_SEC: Boolean;                                                           // 관리자 로그인시 인증번호 체크

  procedure PartComp_Open(Comp: TComponent; sType: String; sDefault: String=''; sValue: String='');

  procedure Send_TC002(sStkCD, sUserID, sAcntNo, sAcntPwd, sBSTP, sOrdTP, sCondTP: String);
...
begin
  ...
  StrToArr(sUserID,                 TC002.GT_HEADER.USER_ID);                   // 사용자ID
  ...
  StrToArr(sAcntNo,                 TC002.ACNT_NO);                             // 계좌번호
  StrToArr(sAcntPwd,                TC002.ACNT_PWD);                            // 계좌 비밀번호
  ...
  StrToArr(__Login_ID,              TC002.MNG_ID);                              // CLIENT/LOSSCUT/BATCH/MNG_ID(강제취소/청산)
  StrToArr(__Local_IP,              TC002.CLIENT_IP);                           // Client IP

  sData := RecordToStr(TC002, SizeOf(TC002));

  DataToServer(MastDB.iwNotiClient, sData + __EOL);
end;
...
begin
  ...
  tmRetry.Enabled := True;
	...
	MsgError('통보서버가 끊어졌습니다. 자동 재접속하겠습니다.\n\n계속적으로 통보가 오지 않으면 재접속하시길 바랍니다.');
...
begin
  ...
  __Find_User1  := Get_INIFile(__Find_FileName, 'FINDSQL', 'USER_1');
...
//  if ArrToStr(Header.PACKET_CD) = 'NC001' then
  ...
  begin
    if __NotiYN then TRPacket_Run('R', Text);
  end;
...
procedure TMastDB.tmRetryTimer(Sender: TObject);
begin
  tmRetry.Enabled := False;
  Set_Log('Server Connect Retry Timer Execute [iwNotiClient]', 'I');

  with iwNotiClient do
  begin
  	try
		  if Not Connected then iwNotiClient.Connected := True;
    except
    end;
  end;
end;
