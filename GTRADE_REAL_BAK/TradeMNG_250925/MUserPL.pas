...
procedure TfmUserPL.chCDClick(Sender: TObject);
begin
  with gdMain do
  begin
  	Columns[GD_CD_CLR_PL].Visible := chCD.Checked;
  	Columns[GD_CD_CMSN].Visible   := chCD.Checked;
  end;
end;
...
procedure TfmUserPL.chHGClick(Sender: TObject);
begin
  with gdMain do
  begin
  	Columns[GD_HG_CLR_PL].Visible := chHG.Checked;
  	Columns[GD_HG_CMSN].Visible   := chHG.Checked;
  end;
end;
...
procedure TfmUserPL.chSIClick(Sender: TObject);
begin
  with gdMain do
  begin
  	Columns[GD_SI_CLR_PL].Visible := chSI.Checked;
  	Columns[GD_SI_CMSN].Visible   := chSI.Checked;
  end;
end;
...
begin
  ...
  PartTableOpen(cbPartner, Format('@|USER_NICK_NM+%s+USER_NM, USER_ID|USER_MST|WHERE USER_GRADE IN (%s) ORDER BY USER_NICK_NM ', [QuotedStr(','),QuotedStr('3')]), '[없음]', 'A');
