...
uses
  ...
  bsribbon, ExtCtrls, RzPanel, UniDAC_Helper, Clipbrd, Menus, bsSkinMenus;
...
procedure TfmFilterIP.IP1Click(Sender: TObject);
begin
  inherited;
	edFind.Text := dbMain.FieldByName('LOGIN_IP').AsString;
  MainTableOpen;
end;

procedure TfmFilterIP.IP3Click(Sender: TObject);
begin
  inherited;
	ClipBoard.AsText := dbMain.FieldByName('LOGIN_IP').AsString;
end;
...
begin
  if edFind.Text = '' then
  begin
//      bsMsgError('검색하실 IP 또는 맥주소를 입력하세요');
    Exit;
  end;
...
//    pnCnt.Caption := FormatFloat('#,##0 건', dbMain.RecordCount);
  finally
    Delay_Hide;
  end;
end;

procedure TfmFilterIP.N2Click(Sender: TObject);
begin
  inherited;
	edFind.Text := dbMain.FieldByName('LOGIN_MAC').AsString;
  MainTableOpen;
end;

procedure TfmFilterIP.N5Click(Sender: TObject);
begin
  inherited;
	ClipBoard.AsText := dbMain.FieldByName('LOGIN_MAC').AsString;
end;
