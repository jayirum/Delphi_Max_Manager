...
procedure TfmRealPL.chAutoSelectClick(Sender: TObject);
begin
...
procedure TfmRealPL.FormActivate(Sender: TObject);
begin
	tmAuto.Enabled := chAutoSelect.Checked;
end;

procedure TfmRealPL.FormDeactivate(Sender: TObject);
begin
  tmAuto.Enabled := False;
end;
...
//    else sUserTp := '';
    ...
    sSQL := Format('SELECT MKT_TP FROM ARTC_MST WHERE ARTC_CD = %s', [QuotedStr('101')]);
    Uni_Open(dbSQL, sSql);
    try
     	if dbSQL.FieldByName('MKT_TP').AsInteger >= 3 then sNgtYN := 'Y';
    except
    end;
...
procedure TfmRealPL.tmAutoTimer(Sender: TObject);
begin
  tmAuto.Enabled := False;
