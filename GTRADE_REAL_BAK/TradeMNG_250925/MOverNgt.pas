...
type
  TfmOverNgt = class(TfmBasic)
    ...
    dbMainNCLR_POS_QTY2: TIntegerField;
  ...
  try
    ...
    sSql := Format( 'SELECT A.USER_ID, ' +
                     ...
                     'WHERE A.ACNT_NO = B.ACNT_NO ' +
                       ...
                       'AND (C.NGT_YN = %s OR ' +
                            'C.NGT_YN = (SELECT CASE WHEN MKT_TP >= 3 THEN %s ELSE %s END FROM ARTC_MST WHERE ARTC_CD = %s) OR ' +
                            '(C.NGT_YN = %s AND E.ARTC_CD IN (%s,%s)) ) ' +
                       'AND A.ARTC_CD = E.ARTC_CD ' +
                       'AND A.OVERNGT_YN = %s ' +
                       'AND (E.BS_TP = %s OR E.BS_TP = %s) ' +
                  'ORDER BY A.USER_ID ',
                    [QuotedStr('S'),
                     QuotedStr('매도'),
                     QuotedStr('매수'),
