...
type
  TfmLogin = class(TForm)
    ...
    procedure edPwd_SecKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
  private
...
var
	sSQL,
  sGrade, sTmp, sPc001, sNm, sNoti, sPwd_Sec: String;
begin
	Result := False;
  __Supervisor := False;
  ...
  with dbSQL do
  begin
    sSQL := 'SELECT CURR_TRADE_DT, CORP_NM, USERSIGN_YN, MINI_TP, PWD_SEC FROM CORP_MST';
    Uni_Open(dbSQL, sSQL);

    __Corp_NM := FieldByName('CORP_NM').AsString;
    __Trade_DT := FieldByName('CURR_TRADE_DT').AsString;
    __UserSignYN := FieldByName('USERSIGN_YN').AsString = 'Y';
    __MINI_YN := FieldByName('MINI_TP').AsString = 'M';
    __PWD_SEC := FieldByName('PWD_SEC').AsString = 'Y';

    if __PWD_SEC then sPwd_Sec := edPwd_Sec.Text
    else sPwd_Sec := '1';

    sSQL := Format('SELECT USER_GRADE, USER_NM, MNG_NOTI_YN FROM USER_MST WHERE USER_ID = %s AND USER_PWD = %s AND ((%s = %s) or USER_SIGN = %s)',
                    [QuotedStr(sID), QuotedStr(sPass), QuotedStr('1'), QuotedStr(sPwd_Sec), QuotedStr(sPwd_Sec)]);

    Uni_Open(dbSQL, sSQL);

    sGrade := FieldByName('USER_GRADE').AsString;
    sNm    := FieldByName('USER_NM').AsString;
    sNoti  := FieldByName('MNG_NOTI_YN').AsString;
  end;
  ...
  begin
    ...
    begin
      if (sGrade = '8') or (sGrade = '9') then   // Manager
      begin
        ...
        if Not (sTmp = 'N') then
        begin
          sSQL := Format('SELECT COUNT(1) AS CNT FROM MNG_ACPT_IP WHERE MIP_IP = %s OR MIP_IP = %s', [QuotedStr(__Local_IP), QuotedStr(GetMacAddress)]);
...
begin
  ...
  if (__PWD_SEC) and (edPwd_Sec.Text = '') then
  begin
  	bsSkinMessage.MessageDlg2('관리자 인증번호를 입력하세요.', '오류', mtError, [mbOK], 0);
  	Exit;
  end;
...
procedure TfmLogin.edPassKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
	if (Key = 13) and (__PWD_SEC = False) then btnLogin.ButtonClick;
end;

procedure TfmLogin.edPwd_SecKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
begin
  sSql := 'SELECT CORP_NM, CORP_TEL, PWD_SEC FROM CORP_MST';
  Uni_Open(dbSQL, sSql);

  __PWD_SEC := dbSQL.FieldByName('PWD_SEC').AsString = 'Y';

  Application.Title := Format('%s 관리자', [dbSQL.FieldByName('CORP_NM').AsString]);
  ...
  if __PWD_SEC then chIDSave.Top := 310;
end;
