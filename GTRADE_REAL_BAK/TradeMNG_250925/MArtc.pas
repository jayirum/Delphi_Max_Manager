...
type
  TfmArtc = class(TfmBasic)
    ...
    dbMainCMSN_TP_NGT: TStringField;
    dbMainCMSN_AMTRT_NGT: TFloatField;
    ...
    dbMainORD_STATE: TStringField;
    dbMainARTC_SORT: TIntegerField;
...
begin
  if bsMsgYesNo(Format('틱가치와 수수료를 ( %s )환율로 변경하시겠습니까?', [edExchRT.Text])) then
  begin
    with dbMain do
    begin
    	if FieldByName('ACNT_TP').AsString = '1' then
      begin
        bsMsgError('국내선물/옵션은 환율적용 불가합니다.');
        Exit;
      end;

      EditWork(dbMain);

	    FieldByName('TICK_VALUE').AsFloat := Trunc(FieldByName('TICK_VALUE_USD').AsFloat * StrToFloatDef(edExchRT.Text, 0));
	    FieldByName('TICK_VALUE_LOW').AsFloat := Trunc(FieldByName('TICK_VALUE_USD').AsFloat * StrToFloatDef(edExchRT.Text, 0));

	    FieldByName('CMSN_AMTRT').AsFloat := Trunc(FieldByName('CMSN_AMTRT_USD').AsFloat * StrToFloatDef(edExchRT.Text, 0));
	    FieldByName('CMSN_AMTRT_NGT').AsFloat := Trunc(FieldByName('CMSN_AMTRT_USD').AsFloat * StrToFloatDef(edExchRT.Text, 0));
...
procedure TfmArtc.bsSkinSpeedButton3Click(Sender: TObject);
var
	sSQL: String;
begin
  if bsMsgYesNo(Format('전종목 틱가치와 수수료를 ( %s )환율로 변경하시겠습니까?', [edExchRT.Text])) then
  begin
    sSQL := Format('UPDATE ARTC_MST SET TICK_VALUE = FLOOR(TICK_VALUE_USD * %g), ' +
                                       'TICK_VALUE_LOW = FLOOR(TICK_VALUE_USD * %g), ' +
                                       'CMSN_AMTRT = FLOOR(CMSN_AMTRT_USD * %g), ' +
                                       'CMSN_AMTRT_NGT = FLOOR(CMSN_AMTRT_USD * %g)' +
                            'WHERE ACNT_TP = %s ',
                    [StrToFloat(edExchRT.Text),
                     StrToFloat(edExchRT.Text),
                     StrToFloat(edExchRT.Text),
                     StrToFloat(edExchRT.Text),
                     QuotedStr('2')
...
begin
  ...
  begin
    ...
    begin
      ...
      if (FieldByName('ACNT_TP').AsString = '2') and (FieldByName('CMSN_TP').AsString = 'R') then
      begin
        FieldByName('CMSN_TP').AsString := 'A';
        FieldByName('CMSN_TP_NGT').AsString := 'A';

        bsMsgInfo('해외선물은 수수료타입은 금액으로 처리가능합니다.');
      end;
    end;
  end;
