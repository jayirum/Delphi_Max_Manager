...
type
  TfmNego_Artc = class(TForm)
    ...
    lbArtcCD_NG: TLabel;
    lbName_NG: TRzPanel;
    btnOK_NG: TbsSkinButton;
    btnCancel_NG: TbsSkinButton;
    lbState_NG: TRzPanel;
    ...
    procedure btnOK_NGClick(Sender: TObject);
    procedure btnCancel_NGClick(Sender: TObject);
  private
...
procedure TfmNego_Artc.btnCancel_NGClick(Sender: TObject);
begin
	DataExec_ArtcUseYN(lbArtcCD_NG.Caption, 'N');
end;
...
procedure TfmNego_Artc.btnOK_NGClick(Sender: TObject);
begin
	DataExec_ArtcUseYN(lbArtcCD_NG.Caption, 'Y');
end;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        else if sArtcCD = 'NG' then
        begin
          pnNG.Visible := True;
          lbName_NG.Caption    := sArtcNM;
          btnOK_NG.Enabled     := sUseYN <> 'Y';
          btnCancel_NG.Enabled := sUseYN = 'Y';
          lbArtcCD_NG.Caption  := sArtcCD;

          if sUseYN = 'Y' then
          begin
          	lbState_NG.Caption := '주문가능';
            lbState_NG.Color := clWhite;
          end
          else
          begin
          	lbState_NG.Caption := '주문불가';
            lbState_NG.Color := $00E1EBFF;
          end;
        end;
