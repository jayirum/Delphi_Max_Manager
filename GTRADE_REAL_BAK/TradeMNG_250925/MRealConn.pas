...
begin
  ...
  try
    ...
    sSql := Format( 'SELECT 0 AS CHECK_TF ' +
                          ...
                          ',B.PART_CD '     +
                          ',B.SERVER_IP '   +
                          ', (SELECT TOP 1 HTS_VER FROM LOGIN_HIS WHERE USER_ID = A.USER_ID ' +
                                'AND LOGIN_TP = %s AND APP_TP = %s ORDER BY LOGIN_DT DESC, LOGIN_TM DESC) AS HTS_VER ' +
                          ', (SELECT TOP 1 LOGIN_MAC FROM LOGIN_HIS WHERE USER_ID = A.USER_ID ' +
                                'AND LOGIN_TP = %s AND APP_TP = %s AND LOGIN_MAC <> %s ORDER BY LOGIN_DT DESC, LOGIN_TM DESC) AS HTS_MAC ' +
