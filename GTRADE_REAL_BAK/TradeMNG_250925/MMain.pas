...
uses
  ...
  bsSkinData, bsSkinHint, BusinessSkinForm, bsribbon, ExtCtrls, TypInfo,
  RzLstBox, bsSkinCtrls, VCL_Helper, DBGridEh, DB, MemDS, DBAccess, Uni,
  kcRaizeCtrl, UniDAC_Helper, Imm, ThdTimer;
...
type
  TfmMain = class(TForm)
    ...
    rgNoti: TbsSkinRadioGroup;
    chRetry: TbsSkinCheckBox;
    btnDataServer: TbsSkinSpeedButton;
    ...
    procedure btnDataServerClick(Sender: TObject);
    ...
    procedure tmInOutTimer(Sender: TObject);
    procedure chInOutRetryClick(Sender: TObject);
    procedure bsSkinSpeedButton9Click(Sender: TObject);
    procedure btnBankPartClick(Sender: TObject);
    procedure bsSkinSpeedButton10Click(Sender: TObject);
    procedure tmMNG_NotiTimer(Sender: TObject);
...
var
  ...
  __MNG_DAT: String;
  BASE_SERVERIP: String = '100.100.100.100';
  UPDATE_DNS1: String = 'trade.co.kr';
  UPDATE_DNS2: String = 'trade.co.kr';
...
uses StdUtils, MUser, MMastDB, MBasic, MAccount, MLogin, MPart, MNego, MInOut,
  ...
  MInOutList, GTNoti, PacketStruct, MPLList, GTUpdate, MRealConn, MLossCut,
  MOverNgt, MHybrid, MBlockIp, MMngIp, MDBTool, MPLClsList, MLogList, MBatchScl,
  MPresent, MPower, MRealPL, MTodayPL, MTermPL, MUserPL, MOverNgt_Req,
  MOverNgtList, MUserOverNgt, MUserNegoARTC, MBankPart, MFilterIP, MMNG_Notice;
...
procedure TfmMain.SetIMEMode(const Language: Integer);
var
...
begin
  Form_Create(TfmInOut, fmInout, 3, 1);
end;
...
begin
  Form_Create(TfmOverNgtList, fmOverNgtList, 3);
end;

procedure TfmMain.btnBankPartClick(Sender: TObject);
begin
  Form_Create(TfmBankPart, fmBankPart, 0);
end;
...
procedure TfmMain.bsSkinSpeedButton10Click(Sender: TObject);
begin
  Form_Create(TfmFilterIP, fmFilterIP, 3);
end;
...
procedure TfmMain.bsSkinSpeedButton2Click(Sender: TObject);
begin
  Form_Create(TfmTodayPL, fmTodayPL, 3, 1);
end;
...
procedure TfmMain.bsSkinSpeedButton4Click(Sender: TObject);
begin
  Form_Create(TfmTermPL, fmTermPL, 3);
end;

procedure TfmMain.bsSkinSpeedButton5Click(Sender: TObject);
begin
  Form_Create(TfmUserPL, fmUserPL, 3);
end;

procedure TfmMain.bsSkinSpeedButton6Click(Sender: TObject);
begin
  Form_Create(TfmOverNgt_Req, fmOverNgt_Req, 3);
end;

procedure TfmMain.bsSkinSpeedButton7Click(Sender: TObject);
begin
  Form_Create(TfmOverNgt, fmOverNgt, 3);
end;

procedure TfmMain.bsSkinSpeedButton8Click(Sender: TObject);
begin
  Form_Create(TfmUserOverNgt, fmUserOverNgt, 3);
end;

procedure TfmMain.bsSkinSpeedButton9Click(Sender: TObject);
begin
  Form_Create(TfmUserNegoARTC, fmUserOverNgt, 3);
end;
...
begin
  Form_Create(TfmPresent, fmPresent, 1, 1);
end;

procedure TfmMain.btnRealConnClick(Sender: TObject);
begin
  Form_Create(TfmRealConn, fmRealConn, 3, 1);
end;

procedure TfmMain.btnRealFClick(Sender: TObject);
begin
	if bsMsgYesNo('국내선물 시세서버를 재실행하시겠습니까?') then
  begin
  	MastDB.PM001_Send('F', 'G_F_REAL_F');
    Sleep(1000);
  	MastDB.PM001_Send('F', 'G_F_REAL_F2');
  end;
end;

procedure TfmMain.btnRealGFClick(Sender: TObject);
begin
	if bsMsgYesNo('해외선물 시세서버를 재실행하시겠습니까?') then
  begin
  	MastDB.PM001_Send('F', 'G_F_REAL_GF');
    Sleep(1000);
  	MastDB.PM001_Send('F', 'G_F_REAL_GF2');
  end;
end;

procedure TfmMain.btnRealPLClick(Sender: TObject);
begin
  Form_Create(TfmRealPL, fmRealPL, 3, 1);
end;
...
procedure TfmMain.btnDataServerClick(Sender: TObject);
begin
	if bsMsgYesNo('데이타서버를 재실행하시겠습니까?') then
  begin
  	MastDB.PM001_Send('F', 'GT_DBS');
  end;
end;
...
begin
  Form_Create(TfmInOutList, fmInOutList, 3, 1);
end;
...
begin
  ...
  begin
    ...
    Set_CFGFile('NOTICE', 'POSITION',    IntToStr(rgNoti.ItemIndex));
    Set_CFGFile('NOTICE', 'RETRY',       BoolToStr(chRetry.Checked));
    Set_CFGFile('PRECENT', 'STK_NAME',   BoolToStr(__PresentName));
    Set_CFGFile('NOTICE', 'INOUT_RETRY', BoolToStr(chInOutRetry.Checked));
...
begin
  iSkin := 6;
   ...
   __MNG_DAT := ChangeFileExt(ParamStr(0), '.DAT');
  ...
  BASE_SERVERIP := Get_CFGFile('G_B_NOTI', 'IP', '', True, __CFGServerIP);
   __DefaultPort := StrToIntDef(Get_CFGFile('G_B_NOTI', 'PORT', '20200', True, __CFGServerIP), 20200);
	...
	__BankCD_Default := StrToIntDef(Get_CFGFile('MUSER', 'BANK_CD', '1', False, __MNG_DAT), 1);
  __OrdParter_Visible := Get_CFGFile('MORDDETAIL', 'VISIBLE', 'N', False, __MNG_DAT);

  __OverNGT_Default := Get_CFGFile('MUSER', 'OVERNGT', 'N', False, __MNG_DAT);
...
begin
  ...
  Caption := Format(' %s 관리자  ( Ver. %s )', [__Corp_NM, _VER]);

  rgNoti.ItemIndex := StrToIntDef(Get_CFGFile('NOTICE', 'POSITION', '1'), 1);
  chRetry.Checked  := StrToBoolDef(Get_CFGFile('NOTICE', 'RETRY', '0'), False);

  __PresentName    := StrToBoolDef(Get_CFGFile('PRECENT', 'STK_NAME', '0'), False);

  chInOutRetry.Checked := StrToBoolDef(Get_CFGFile('NOTICE', 'INOUT_RETRY', '0'), False);

  tmInOut.Enabled  := False;
  tmInOut.Interval := 1 * 60 * 1000;
	tmInOut.Enabled := chInOutRetry.Checked;
  ...
  tmMNG_Noti.Enabled := True;

  Exit;
//	sSql := 'SELECT * FROM NOTICE_SYS WHERE NOTICE_VIEW = 1';
//  Uni_Open(dbSQL, sSql);
//
//  with dbSQL do
//  begin
...
procedure TfmMain.pnCorpDblClick(Sender: TObject);
var
  sText: String;
begin
  sText := UpperCase(InputBox('GT Solution', 'Notice', ''));

	if sText = 'GTNOTI' then MNGNoti_Run(1);
end;
...
procedure TfmMain.tmInOutTimer(Sender: TObject);
var
	iCnt: Integer;
	sSQL: String;
begin
  tmInOut.Enabled := False;
	...
	sSql := Format('SELECT COUNT(1) AS CNT FROM INOUT WHERE RSLT_TP = %s', [QuotedStr('0')]);
  Uni_Open(dbSQL, sSql);
  ...
  begin
    ...
    if iCnt > 0 then
    	Noti_Show(NowDateMSecTime, '입출금요청', Format('미처리된 입출금내역이 ( %d ) 건이 있습니다.', [iCnt]), 'fmInOut', __Corp_NM, ExtractFilePath(ParamStr(0)) + 'SOUND\Inout.wav', rgNoti.ItemIndex+1, chRetry.Checked);  // 입출금.wav

		Close;
  end;

	tmInOut.Enabled := True;
end;

procedure TfmMain.tmMNG_NotiTimer(Sender: TObject);
var
	iCnt: Integer;
	sSQL: String;
begin
  tmMNG_Noti.Enabled := False;

	sSql := Format('SELECT COUNT(1) AS CNT FROM MNG_NOTICE WHERE MNG_VIEW = %s', [QuotedStr('0')]);
  Uni_Open(dbSQL, sSql);
...
procedure TfmMain.chInOutRetryClick(Sender: TObject);
begin
  tmInOut.Enabled := chInOutRetry.Checked;
end;
...
var
  ...
  NL002 : TNL002;
  NB001 : TNB001;
  sPacket, sData, sTm, sMsg, sIOTP, sWav: String;
begin
  inherited;
  if Message.Msg = WM_USER_NOTI_DATA then                                       // Real Packet
    ...
    begin
      ...
      sIOTP := ArrToStr(NC001.IO_TP);

      sWav := 'Inout.wav';                                                      // 입출금.wav

      if sIOTP = '1' then sWav := 'In.wav'                                      // 입금통보.wav
      else if sIOTP = '2' then sWav := 'Out.wav';                               // 출금통보.wav
      

      Noti_Show(sTm, '입출금요청', ArrToStr(NC001.NOTI_MSG), 'fmInOut', __Corp_NM, ExtractFilePath(ParamStr(0)) + 'SOUND\'+ sWav, rgNoti.ItemIndex+1, chRetry.Checked);
    end
    else if sPacket = 'NC002' then
    begin
      StrToRecord(sData, NC002, SizeOf(NC002));
      sTm := ArrToStr(NC002.GT_HEADER.TM);
      Noti_Show(sTm, '오버나잇신청', ArrToStr(NC002.NOTI_MSG), 'fmOverNgt_Req', __Corp_NM, ExtractFilePath(ParamStr(0)) +'SOUND\OverNgt.wav', rgNoti.ItemIndex+1, chRetry.Checked);  // 오버나잇신청.wav
    end
    else if sPacket = 'NC003' then                                              // 신규회원가입
    begin
      StrToRecord(sData, NC003, SizeOf(NC003));
      sTm := ArrToStr(NC003.GT_HEADER.TM);
      Noti_Show(sTm, ArrToStr(NC003.NOTI_TITLE), ArrToStr(NC003.NOTI_MSG), 'fmUser', __Corp_NM, ExtractFilePath(ParamStr(0)) +'SOUND\NewUser.wav', rgNoti.ItemIndex+1, chRetry.Checked);  // 회원가입.wav
    end
    else if sPacket = 'NL002' then                                              // 로스컷통보
    begin
      ...
      sTm  := ArrToStr(NL002.GTN_HEADER.GT_HEADER.TM);
    	...
    	Noti_Show(sTm, '로스컷 통보', sMsg, '', __Corp_NM, ExtractFilePath(ParamStr(0)) +'SOUND\Losscut.wav', rgNoti.ItemIndex+1, chRetry.Checked);   // 로스컷.wav
    end
    else if sPacket = 'NB001' then
    begin
      ...
      Noti_Show(sTm, sMsg, ArrToStr(NB001.NOTI_MSG), '', __Corp_NM, '', rgNoti.ItemIndex+1, chRetry.Checked);
    end;
  end
  ...
  begin
    ...
    else if PChar(Message.WParam) = 'fmOverNgt_Req' then
    begin
      if Not Check_FormCreate('fmOverNgt_Req') then Form_Create(TfmOverNgt_Req , fmOverNgt_Req, 3)
      else
      begin
        with fmOverNgt_Req do
        begin
          Show;
          TbsSkinSpeedButton(FindComponent('btnFilter')).ButtonClick;
        end;
    ...
    else if PChar(Message.WParam) = 'fmUser' then
    begin
      if Not Check_FormCreate('fmUser') then Form_Create(TfmUser , fmUser, 3)
      else
      begin
        with fmUser do
        begin
...
//          TbsSkinSpeedButton(FindComponent('btnFilter')).ButtonClick;
    ...
    else if PChar(Message.WParam) = 'fmHybrid' then
    begin
