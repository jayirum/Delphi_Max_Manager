...
begin
  ...
  if edFind.Text <> '' then sNm := Format(' AND (A.USER_NM LIKE %s OR A.USER_ID LIKE %s) ', [QuotedStr('%'+edFind.Text+'%'), QuotedStr('%'+edFind.Text+'%')]);
...
begin
  try
    ...
    sSql := Format( 'SELECT A.USER_ID, ' +
  	...
  	sSql := sSql + _sMainWhere + sUserTp + 'ORDER BY A.USER_ID, A.RQST_TRADE_DT DESC, A.RQST_TM DESC';
