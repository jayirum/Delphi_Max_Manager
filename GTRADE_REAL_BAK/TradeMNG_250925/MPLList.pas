...
type
  TfmPLList = class(TfmBasic)
    ...
    procedure gdClsPLCellClick(Column: TColumnEh);
  private
...
var
  sPLSql, sPLClsSql, sSQL, sAcntTp, sWhere , sUnion, sUserTp, sUserTp1 : String;
  bNowDt : BOOLEAN;
begin
...
//    else sUserTp := '';
    ...
    begin
      if __Supervisor then
      begin
        if cbUserGrade.ItemIndex <> 0 then
        begin
        	sUserTp  := Format(' AND B1.USER_GRADE = %s ', [IntToStr(cbUserGrade.ItemIndex)]);
        	sUserTp1 := Format(' AND A1.USER_GRADE = %s ', [IntToStr(cbUserGrade.ItemIndex)]);
        end
        else if chUserTp.Checked then
        begin
        	sUserTp  := Format(' AND B1.USER_GRADE IN (%s,%s) ', [QuotedStr('2'),QuotedStr('7')]);
        	sUserTp1 := Format(' AND A1.USER_GRADE IN (%s,%s) ', [QuotedStr('2'),QuotedStr('7')]);
        end;
      end
      else
      begin
        ...
        begin
        	sUserTp  := ' AND B1.USER_GRADE = 2 ';
        	sUserTp1 := ' AND A1.USER_GRADE = 2 ';
        end
        else
        begin
        	sUserTp  := ' AND B1.USER_GRADE <> 7 ';
        	sUserTp1 := ' AND A1.USER_GRADE <> 7 ';
        end;
      end;
    end;
    ...
    sPLSql := Format( 'SELECT * FROM ( '+
                      ...
                      '         , MAX(ISNULL(A.ACNT_AMT,0)) AS ACNT_AMT '+
                      '         ,(SELECT CODE_VALUE_NM FROM CODE_MST WHERE CODE_ID = %s AND CODE_VALUE = A.ACNT_TP) AS ACNT_TP '+
                      '         , A.ACNT_NO '+
                      '         , A.USER_ID '+
                      '         , A.USER_NM '+
                      '         , MAX(ISNULL(A.BF_ACNT_AMT,0)) AS ACNT_AMT_BF '+
                      '         , MAX(ISNULL((A.ACNT_AMT + A.CLR_PL - A.CMSN),0)) AS ACNT_AMT_TODAY '+
...
procedure TfmPLList.gdClsPLCellClick(Column: TColumnEh);
begin
  inherited;
