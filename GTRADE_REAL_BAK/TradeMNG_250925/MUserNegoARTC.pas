...
begin
	sSQL := Format('SELECT A.USER_ID, ' +
                  ...
                  'WHERE A.USER_ID = B.USER_ID ' +
                    ...
                    'AND (%s = %s OR B.ARTC_USE_YN = %s) ',
                                                        [QuotedStr('0'),
                                                         QuotedStr(cbArtcCD.Value),
                                                         QuotedStr(cbArtcCD.Value),
                                                         QuotedStr('0'),
                                                         QuotedStr(cbYN.Value),
                                                         QuotedStr(cbYN.Value)]);
...
begin
  ...
  PartTableOpen(cbArtcCD, Format('@|ARTC_NM, ARTC_CD|ARTC_MST|WHERE ACNT_TP=%s', [QuotedStr('2')]), '[ÀüÃ¼]', '0');

  cbArtcCD.ItemIndex :=0 ;
//  MainTableOpen;
//  UserTableOpen;
