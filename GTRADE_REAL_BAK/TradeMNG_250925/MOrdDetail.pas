...
type
  TfmOrdDetail = class(TfmBasic)
    ...
    btnOnePartner: TbsSkinSpeedButton;
    ...
    procedure btnOnePartnerClick(Sender: TObject);
  private
...
procedure TfmOrdDetail.btnOnePartnerClick(Sender: TObject);
var
	sSQL: String;
begin
  inherited;
  ...
  if _User_ID = '' then
  begin
    bsMsgError('파트너 변경할 회원을 먼저 선택하세요.');
    Exit;
  end;
  ...
  if bsMsgYesNo(Format('( %s / 체결번호 = %d ) - 파트너를 [ %s ]로 변경하시겠습니까?',
                         [_User_ID, dbMain.FieldByName('CNTR_NO').AsInteger, cbPartner.Text])) then
  begin
    sSql := Format('UPDATE CNTR SET PARTNER_ID = %s WHERE USER_ID = %s AND CNTR_NO = %d',
                        [QuotedStr(cbPartner.Value),
                         QuotedStr(_User_ID),
                         dbMain.FieldByName('CNTR_NO').AsInteger]);
...
begin
  ...
  if bsMsgYesNo(Format('( %s ) - 파트너를 [ %s ]로 변경하시겠습니까? \n\n회원관리에 파트너도 함께 변경됩니다.', [_User_ID, cbPartner.Text])) then
  begin
...
begin
  ...
  lbParter.Visible      := __OrdParter_Visible = 'Y';
  cbPartner.Visible     := __OrdParter_Visible = 'Y';
  btnPartner.Visible    := __OrdParter_Visible = 'Y';
  btnOnePartner.Visible := __OrdParter_Visible = 'Y';
  ...
  PartTableOpen(cbPartner, Format('@|USER_NICK_NM+%s+USER_NM, USER_ID|USER_MST|WHERE USER_GRADE IN (%s) ORDER BY USER_NICK_NM ', [QuotedStr(','),QuotedStr('3')]), '', '0');
...
//     PartTableOpen(TComponent(gdCntr.Columns[27]), Format('@|CODE_VALUE_NM, CODE_VALUE|CODE_MST|WHERE CODE_ID = %s', [QuotedStr('API_TP')]));
    ...
    _User_ID := dbUser.FieldByName('USER_ID').AsString;
    ...
    begin
      sSQL := Format('SELECT SUM(CLR_PL_SUM) AS CLR_PL, SUM(CMSN_SUM) AS CMSN FROM ACNT_CLS WHERE TRADE_DT = %s AND USER_ID = %s',
                      [QuotedStr(StrReplace(dtCntrDt.Text, '-', '')),
                       QuotedStr(dbUser.FieldByName('USER_ID').AsString)]);
    end;
