...
uses
  ...
  Dialogs, bsMessages, BusinessSkinForm, bsSkinCtrls, ExtCtrls, RzPanel, DB,
  MemDS, DBAccess, Uni, VCL_Helper, StdCtrls;
...
begin
  ...
  sRslt := AcntMakeProc(intTostr(rgAcntTp.ItemIndex));
...
begin
    ...
    if sType = '0' then sWhere := '(SELECT GUJA_MIN_AMT_1 FROM CORP_MST)'
    else if sType = '2' then sWhere := '(SELECT GUJA_MIN_AMT_2 FROM CORP_MST)';

    if Not __MINI_YN then iLvg := 1
    else iLvg := 10;
    ...
    sSql := Format( ' INSERT INTO ACNT_MST   ' +
                    ...
                    '  VALUES                ' +
                    ' (%s,%s,%s,%s,%s,%s,%s,%s,%d,%s,(SELECT DUP_YN FROM CORP_MST)) ',
