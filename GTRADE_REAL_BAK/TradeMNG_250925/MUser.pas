...
uses
  ...
  Clipbrd, DBGridEhImpExp, RzDBChk;
...
type
  TfmUser = class(TfmBasic)
    ...
    dbAcntPROFITCUT_AMT: TFloatField;
    dbAcntPLAMT: TFloatField;
    btnPwd_Sec: TbsSkinSpeedButton;
    lbPwd_Sec: TbsSkinLabel;
    edPwd_Sec: TkcRzDBEdit;
    N6: TMenuItem;
    N7: TMenuItem;
    bsSkinLabel48: TbsSkinLabel;
    edLvg: TkcRzDBEdit;
    btnLvg: TbsSkinButton;
    pnLvg: TRzPanel;
    RzPanel9: TRzPanel;
    chLvg1: TRzCheckBox;
    chLvg2: TRzCheckBox;
    ...
    chLvg20: TRzCheckBox;
    chLvg50: TRzCheckBox;
    chLvg100: TRzCheckBox;
    chLvg200: TRzCheckBox;
    btnSave: TbsSkinSpeedButton;
    bsSkinSpeedButton1: TbsSkinSpeedButton;
    dbAcntLEVERAGE_YN: TStringField;
    dbAcntLEVERAGE_MANUAL: TStringField;
    chLvg: TRzDBCheckBox;
    ...
    procedure btnLvgClick(Sender: TObject);
    procedure chLvgClick(Sender: TObject);
  private
...
procedure TfmUser.btnLvgClick(Sender: TObject);
var
  i: Integer;
  sLvg: String;
begin
  chLvg1.Checked := False;
  chLvg2.Checked := False;
  chLvg5.Checked := False;
  chLvg10.Checked := False;
  chLvg20.Checked := False;
  chLvg50.Checked := False;
  chLvg100.Checked := False;
  chLvg200.Checked := False;

  for i := 1 to 10 do
  begin
    sLvg := Get_Delimiter(dbAcnt.FieldByName('LEVERAGE_MANUAL').AsString, i, '/');

    if sLvg = '1' then   chLvg1.Checked := True;
    if sLvg = '2' then   chLvg2.Checked := True;
    if sLvg = '5' then   chLvg5.Checked := True;
    if sLvg = '10' then  chLvg10.Checked := True;
    if sLvg = '20' then  chLvg20.Checked := True;
    if sLvg = '50' then  chLvg50.Checked := True;
    if sLvg = '100' then chLvg100.Checked := True;
    if sLvg = '200' then chLvg200.Checked := True;
  end;
...
procedure TfmUser.btnCloseClick(Sender: TObject);
begin
  pnLvg.Visible := False;
end;
...
var
  sSql, sAcntTp, sLvgYN, sLvgSQL: String;
  sGujaCnt: Integer;
begin
  Case pgMain.ActivePageIndex of
    0 : begin
    ...
    1 : begin
          ...
          if chLvg.Checked then
          begin
            sLvgYN := 'Y';

            if dbAcnt.FieldByName('LEVERAGE_MANUAL').AsString = '' then sLvgYN := 'N';
          end
          else sLvgYN := 'N';

//          if sLvgYN = 'N' then
//          begin
//            sSql := 'SELECT MAX(LEVERAGE) AS LEVERAGE FROM LVG_MST';
//
//            Uni_Open(MastDB.dbSQL, sSql);
//
//            sLvgSQL := Format(', LEVERAGE = %d ', [MastDB.dbSQL.FieldByName('LEVERAGE').AsInteger]);
//          end;
...
var
  sSQL, sAcntNo, sLvg, sLast: String;
begin
  sLvg := '';

  if chLvg1.Checked then
  begin
    sLvg := sLvg + '1/';
    sLast := '1';
  end;

  if chLvg2.Checked then
  begin
    sLvg := sLvg + '2/';
    sLast := '2';
  end;

  if chLvg5.Checked then
  begin
    sLvg := sLvg + '5/';
    sLast := '5';
  end;

  if chLvg10.Checked then
  begin
    sLvg := sLvg + '10/';
    sLast := '10';
  end;

  if chLvg20.Checked then
  begin
    sLvg := sLvg + '20/';
    sLast := '20';
  end;

  if chLvg50.Checked then
  begin
    sLvg := sLvg + '50/';
    sLast := '50';
  end;

  if chLvg100.Checked then
  begin
    sLvg := sLvg + '100/';
    sLast := '100';
  end;

  if chLvg200.Checked then
  begin
    sLvg := sLvg + '200/';
    sLast := '200';
  end;
  ...
  if sLvg = '' then
  begin
    bsMsgError('한개이상 회원별 레버리지를 선택하세요.!');
    Exit;
  end;
  ...
  begin
    ...
    // 접속 있는 경우
    sSql := Format('SELECT CONN_YN FROM ACNT_MST WHERE ACNT_NO = %s ',
                    [QuotedStr(sAcntNo)]);

    Uni_Open(MastDB.dbSQL, sSql);

    if MastDB.dbSQL.FieldByName('CONN_YN').AsString = 'Y' then
    begin
      MsgError('접속되어 있는 경우 회원별 레버리지 변경이 불가능합니다.');
      pnLvg.Visible := False;
    ...
    // 포지션 있는 경우
    sSql := Format('SELECT COUNT(1) AS CNT FROM NCLR_POS WHERE ACNT_NO = %s ',
                    [QuotedStr(sAcntNo)]);
    ...
    begin
      MsgError('포지션이 있는 경우 회원별 레버리지 변경이 불가능합니다.');
      pnLvg.Visible := False;
    ...
    // 미체결 있는 경우
    sSql := Format('SELECT COUNT(1) AS CNT FROM ORD WHERE ACNT_NO = %s ' +
                                                     'AND ACPT_TP IN (%s, %s, %s) ' +
                                                     'AND REMN_QTY > 0 ' +
                                                     'AND COND_TP <> %s ',
                    [QuotedStr(sAcntNo), QuotedStr('1'), QuotedStr('2'), QuotedStr('4'), QuotedStr('4')]);

    Uni_Open(MastDB.dbSQL, sSql);

    if MastDB.dbSQL.FieldByName('CNT').AsInteger > 0 then
    begin
      MsgError('미체결이 있는 경우 회원별 레버리지 변경이 불가능합니다.');
      pnLvg.Visible := False;
    ...
    FieldByName('LEVERAGE_MANUAL').AsString := sLvg;
    FieldByName('LEVERAGE').AsInteger := StrToIntDef(sLast, 10);
...
procedure TfmUser.chLvgClick(Sender: TObject);
var
  i: Integer;
  sAcntNo, sSql, sLvg: String;
begin
  Exit;

  if chLvg.Tag = 1 then Exit;

  if chLvg.Checked then
  begin
    if dbAcnt.FieldByName('LEVERAGE_MANUAL').AsString = '' then
    begin
      MsgError('한개이상 회원별 레버리지를 선택하셔야 합니다.');
  ...
  begin
    ...
    begin
      ...
      chLvg.Tag := 1;
      chLvg.Checked := Not chLvg.Checked;
      chLvg.Tag := 0;
    end;
  ...
  begin
    ...
    begin
      ...
      if sLvg <> '' then
      begin
        cbLvg.Items.Add(sLvg);
        cbLvg.Values.Add(sLvg);
      end
      else Break;
    end;
  end
...
begin
  ...
  begin
    ...
    begin
      ...
      if (FieldByName('LEVERAGE_YN').OldValue <> FieldByName('LEVERAGE_YN').NewValue) then
      begin
        sAcntNo := FieldByName('ACNT_NO').AsString;
        ...
        begin
          ...
          chLvg.Tag := 1;
          FieldByName('LEVERAGE_YN').AsString := FieldByName('LEVERAGE_YN').OldValue;
          chLvg.Tag := 0;
        end;
        ...
        if FieldByName('LEVERAGE_YN').AsString = 'Y' then
        begin
          bMatch := False;
          ...
          begin
            ...
            if sLvg <> '' then
            begin
              if Trim(sLvg) = Trim(FieldByName('LEVERAGE').AsString) then
              begin
                bMatch := True;
                Break;
              end;

              iLvgSQL := StrToIntDef(sLvg, 10);
            end
            else Break;
          end;

          if Not bMatch then FieldByName('LEVERAGE').AsInteger := iLvgSQL;
        end
        else
        begin
          sSql := Format('SELECT COUNT(1) AS CNT FROM LVG_MST WHERE LEVERAGE = %d', [FieldByName('LEVERAGE').AsInteger]);
          Uni_Open(MastDB.dbSQL, sSql);
          ...
          begin
            sSql := 'SELECT MAX(LEVERAGE) AS LEVERAGE FROM LVG_MST';
            Uni_Open(MastDB.dbSQL, sSql);
            iLvgSQL := MastDB.dbSQL.FieldByName('LEVERAGE').AsInteger;

            FieldByName('LEVERAGE').AsInteger := iLvgSQL;
          end;
        end;
      end;
    end;
...
var
  sSQL, sServerIP: String;
begin
  inherited;

  sSQL := 'SELECT SERVER_IP FROM SERVER_MST';
  Uni_Open(MastDB.dbSQL, sSQL);
  sServerIP := MastDB.dbSQL.FieldByName('SERVER_IP').AsString;
  ...
  begin
    ...
    FieldByName('SERVER_IP').AsString := sServerIP;
...
//    chLvg.Checked := dbAcnt.FieldByName('LEVERAGE_YN').AsString = 'Y';
...
//  LoginCnt;
  ...
  chLvg.Enabled := __MINI_YN;
  edLvg.Enabled := __MINI_YN;
  btnLvg.Enabled := __MINI_YN;

  if Not __Supervisor then edPass.PasswordChar := #7;

  lbPwd_Sec.Visible  := __PWD_SEC;
  edPwd_Sec.Visible  := __PWD_SEC;
  btnPwd_Sec.Visible := __PWD_SEC;
end;
...
procedure TfmUser.N7Click(Sender: TObject);
begin
  with dgSave do
  begin
    InitialDir := ExtractFilePath(ParamStr(0));
  	...
  	if Execute then
    begin
			SaveDBGridEhToExportFile(TDBGridEhExportAsOLEXLS, gdLogin, FileName, True);
    end;
  end;
end;
