inherited fmIPList: TfmIPList
  Tag = 3008
  Caption = 'IP / '#47589#51452#49548' '#51312#54924
  OnShow = FormShow
  ExplicitWidth = 945
  ExplicitHeight = 580
  PixelsPerInch = 96
  TextHeight = 12
  inherited pnTop: TRzPanel
    inherited btnFilter: TbsSkinSpeedButton
      Left = 546
      Top = 4
      Height = 23
      ExplicitLeft = 546
      ExplicitTop = 4
      ExplicitHeight = 23
    end
    inherited btnInsert: TbsSkinSpeedButton
      Left = 765
      Top = 0
      Visible = False
      ExplicitLeft = 765
      ExplicitTop = 0
    end
    inherited btnEdit: TbsSkinSpeedButton
      Left = 765
      Top = 0
      Visible = False
      ExplicitLeft = 765
      ExplicitTop = 0
    end
    inherited btnDelete: TbsSkinSpeedButton
      Left = 765
      Top = 0
      Visible = False
      ExplicitLeft = 765
      ExplicitTop = 0
    end
    inherited btnPost: TbsSkinSpeedButton
      Left = 772
      Top = 6
      Visible = False
      ExplicitLeft = 772
      ExplicitTop = 6
    end
    inherited bsRibbonDivider4: TbsRibbonDivider
      Left = 772
      Top = -2
      Visible = False
      ExplicitLeft = 772
      ExplicitTop = -2
    end
    inherited btnExcel: TbsSkinSpeedButton
      Left = 773
      Top = 4
      Height = 23
      Anchors = [akTop, akRight]
      OnClick = btnExcelClick
      ExplicitLeft = 773
      ExplicitTop = 4
      ExplicitHeight = 23
    end
    inherited nvMain: TRzDBNavigator
      Left = 669
      Hints.Strings = ()
      Anchors = [akTop, akRight]
      ExplicitLeft = 669
    end
    object pnCnt: TRzPanel
      Left = 552
      Top = 4
      Width = 113
      Height = 23
      Anchors = [akTop, akRight]
      BorderOuter = fsFlat
      Color = clWhite
      FlatColor = 8404992
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = #44404#47548#52404
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 2
    end
    object dtDt: TkcRzDateTimeEdit
      Left = 124
      Top = 5
      Width = 121
      Height = 20
      CalendarElements = [ceYear, ceMonth, ceArrows, ceDaysOfWeek, ceFillDays, ceTodayButton]
      CaptionTodayBtn = #50724#45720#47196
      EditType = etDate
      FrameController = MastDB.RzFrameController
      ImeName = 'Microsoft Office IME 2007'
      TabOrder = 3
      EnterToTab = True
    end
    object dtEnd: TkcRzDateTimeEdit
      Left = 249
      Top = 5
      Width = 121
      Height = 20
      CalendarElements = [ceYear, ceMonth, ceArrows, ceDaysOfWeek, ceFillDays, ceTodayButton]
      CaptionTodayBtn = #50724#45720#47196
      EditType = etDate
      FrameController = MastDB.RzFrameController
      ImeName = 'Microsoft Office IME 2007'
      TabOrder = 4
      EnterToTab = True
    end
    object edFind: TRzEdit
      Tag = 500
      Left = 424
      Top = 5
      Width = 122
      Height = 20
      FrameController = MastDB.RzFrameController
      ImeName = 'Microsoft IME 2010'
      TabOrder = 5
    end
  end
  object gdMain: TDBGridEh [2]
    Left = 0
    Top = 31
    Width = 937
    Height = 500
    TabStop = False
    Align = alClient
    ColumnDefValues.Layout = tlCenter
    ColumnDefValues.Title.TitleButton = True
    Ctl3D = False
    DataSource = dsMain
    DynProps = <>
    FixedColor = 15790320
    Flat = True
    ImeName = 'Microsoft IME 2010'
    IndicatorOptions = [gioShowRowIndicatorEh, gioShowRecNoEh]
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
    OptionsEh = [dghFixed3D, dghHighlightFocus, dghClearSelection, dghAutoSortMarking, dghDialogFind, dghShowRecNo, dghColumnResize, dghColumnMove, dghExtendVertLines]
    ParentCtl3D = False
    RowDetailPanel.Color = clBtnFace
    RowHeight = 21
    TabOrder = 2
    TitleParams.Color = clBtnFace
    TitleParams.RowHeight = 21
    TitleParams.FillStyle = cfstGradientEh
    OnTitleBtnClick = gdMainTitleBtnClick
    Columns = <
      item
        Alignment = taCenter
        DynProps = <>
        EditButtons = <>
        FieldName = 'LOG_SVR_DT'
        Footers = <>
        Title.Alignment = taCenter
        Title.Caption = #51068#51088
        Title.Color = 16250871
        Width = 65
      end
      item
        Alignment = taCenter
        DynProps = <>
        EditButtons = <>
        FieldName = 'LOG_SVR_TM'
        Footers = <>
        Title.Alignment = taCenter
        Title.Caption = #49884#44036
        Title.Color = 16250871
        Width = 80
      end
      item
        Alignment = taCenter
        DynProps = <>
        EditButtons = <>
        FieldName = 'ACNT_NO'
        Footers = <>
        Title.Alignment = taCenter
        Title.Caption = #44228#51340#48264#54840
        Title.Color = 16250871
        Width = 80
      end
      item
        Alignment = taCenter
        DynProps = <>
        EditButtons = <>
        FieldName = 'LOG_TP'
        Footers = <>
        Title.Alignment = taCenter
        Title.Caption = #47196#44536#50976#54805
        Title.Color = 16250871
        Width = 150
      end
      item
        Alignment = taCenter
        Color = 16250879
        DynProps = <>
        EditButtons = <>
        FieldName = 'STK_CD'
        Footers = <>
        Title.Alignment = taCenter
        Title.Caption = #51333#47785#53076#46300
        Title.Color = 16250871
        Width = 65
      end
      item
        Alignment = taCenter
        DynProps = <>
        EditButtons = <>
        FieldName = 'BS_TP'
        Footers = <>
        Title.Alignment = taCenter
        Title.Caption = #44396#48516
        Title.Color = 16250871
        Width = 50
      end
      item
        Alignment = taCenter
        DynProps = <>
        EditButtons = <>
        FieldName = 'NOW_PRC'
        Footers = <>
        Title.Alignment = taCenter
        Title.Caption = #54788#51116#44032
        Title.Color = 16250871
        Width = 60
      end
      item
        Alignment = taCenter
        DynProps = <>
        EditButtons = <>
        FieldName = 'ORD_PRC'
        Footers = <>
        Title.Alignment = taCenter
        Title.Caption = #51452#47928#44032
        Title.Color = 16250871
        Width = 60
      end
      item
        Alignment = taCenter
        DynProps = <>
        EditButtons = <>
        FieldName = 'ORD_QTY'
        Footers = <>
        Title.Alignment = taCenter
        Title.Caption = #49688#47049
        Title.Color = 16250871
        Width = 45
      end
      item
        Alignment = taCenter
        DynProps = <>
        EditButtons = <>
        FieldName = 'HOGA_NO'
        Footers = <>
        Title.Alignment = taCenter
        Title.Caption = #54840#44032
        Title.Color = 16250871
        Width = 60
      end
      item
        DynProps = <>
        EditButtons = <>
        FieldName = 'LOG_MSG'
        Footers = <>
        Title.Alignment = taCenter
        Title.Caption = #47196#44536#45236#50857
        Title.Color = 16250871
        Width = 400
      end
      item
        Alignment = taCenter
        DynProps = <>
        EditButtons = <>
        FieldName = 'LOG_IP'
        Footers = <>
        Title.Alignment = taCenter
        Title.Caption = #47196#44536'IP'
        Title.Color = 16250871
        Width = 100
      end
      item
        Alignment = taCenter
        DynProps = <>
        EditButtons = <>
        FieldName = 'LOG_CLT_DT'
        Footers = <>
        Title.Alignment = taCenter
        Title.Caption = 'C_'#51068#51088
        Title.Color = 16250871
        Width = 65
      end
      item
        Alignment = taCenter
        DynProps = <>
        EditButtons = <>
        FieldName = 'LOG_CLT_TM'
        Footers = <>
        Title.Alignment = taCenter
        Title.Caption = 'C_'#49884#44036
        Title.Color = 16250871
        Width = 80
      end>
    object RowDetailData: TRowDetailPanelControlEh
    end
  end
  object edUserID: TkcRzDBEdit [3]
    Left = 649
    Top = 5
    Width = 150
    Height = 20
    DataSource = dsUser
    DataField = 'USER_ID'
    ReadOnly = True
    FrameController = MastDB.RzFrameController
    ImeMode = imSAlpha
    ImeName = 'Microsoft IME 2010'
    TabOrder = 3
    Visible = False
    EnterToTab = True
  end
  inherited imgBtn: TImageList
    Bitmap = {
      
      000000000000}
  end
  inherited dbMain: TUniQuery
    SQL.Strings = (
      'SELECT'
      ' ACNT_NO'
      ',LOG_CLT_DT'
      ',LOG_CLT_TM'
      ',LOG_IP'
      ',USER_ID'
      ',USER_NM'
      ',LOG_TP'
      ',STK_CD'
      ',BS_TP'
      ',NOW_PRC'
      ',ORD_PRC'
      ',ORD_QTY'
      ',HOGA_NO'
      ',LOG_SVR_DT'
      ',LOG_SVR_TM'
      ',LOG_MSG '
      ',COUNT(1) OVER() AS TOTCNT'
      'FROM USER_LOG')
    FetchRows = 50
    SpecificOptions.Strings = (
      'SQL Server.FetchAll=False')
    object dbMainACNT_NO: TStringField
      FieldName = 'ACNT_NO'
      FixedChar = True
      Size = 11
    end
    object dbMainLOG_CLT_DT: TStringField
      FieldName = 'LOG_CLT_DT'
      FixedChar = True
      Size = 8
    end
    object dbMainLOG_CLT_TM: TStringField
      FieldName = 'LOG_CLT_TM'
      FixedChar = True
      Size = 12
    end
    object dbMainLOG_IP: TStringField
      FieldName = 'LOG_IP'
      FixedChar = True
      Size = 15
    end
    object dbMainUSER_ID: TStringField
      FieldName = 'USER_ID'
    end
    object dbMainUSER_NM: TStringField
      FieldName = 'USER_NM'
    end
    object dbMainLOG_TP: TStringField
      FieldName = 'LOG_TP'
      FixedChar = True
      Size = 3
    end
    object dbMainSTK_CD: TStringField
      FieldName = 'STK_CD'
      Size = 10
    end
    object dbMainBS_TP: TStringField
      FieldName = 'BS_TP'
      FixedChar = True
      Size = 1
    end
    object dbMainNOW_PRC: TFloatField
      FieldName = 'NOW_PRC'
    end
    object dbMainORD_PRC: TFloatField
      FieldName = 'ORD_PRC'
    end
    object dbMainORD_QTY: TIntegerField
      FieldName = 'ORD_QTY'
    end
    object dbMainHOGA_NO: TIntegerField
      FieldName = 'HOGA_NO'
    end
    object dbMainLOG_SVR_DT: TStringField
      FieldName = 'LOG_SVR_DT'
      FixedChar = True
      Size = 8
    end
    object dbMainLOG_SVR_TM: TStringField
      FieldName = 'LOG_SVR_TM'
      FixedChar = True
      Size = 12
    end
    object dbMainLOG_MSG: TStringField
      FieldName = 'LOG_MSG'
      Size = 50
    end
    object dbMainTOTCNT: TIntegerField
      FieldName = 'TOTCNT'
      ReadOnly = True
    end
  end
  inherited dbPart: TUniQuery
    Left = 161
  end
  object dsUser: TDataSource
    DataSet = dbUser
    Left = 160
    Top = 384
  end
  object dbUser: TUniQuery
    Connection = MastDB.UniConnection
    SQL.Strings = (
      'SELECT USER_NM, USER_ID'
      'FROM USER_MST')
    FetchRows = 50
    SpecificOptions.Strings = (
      'SQL Server.FetchAll=False')
    Left = 113
    Top = 384
    object dbUserUSER_NM: TStringField
      FieldName = 'USER_NM'
    end
    object dbUserUSER_ID: TStringField
      FieldName = 'USER_ID'
      Required = True
    end
  end
end
