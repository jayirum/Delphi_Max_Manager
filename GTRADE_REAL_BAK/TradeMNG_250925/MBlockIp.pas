...
procedure TfmBlockIp.btnFindClick(Sender: TObject);
var
  sSql : String;
begin
  try
    ...
    sSql := Format('SELECT IP_SEQ '+
                        ', UIP_IP '+
                        ', UIP_BIGO '+
                     'FROM USER_BLOCK_IP ' +
                    'WHERE UIP_IP LIKE %s ' +
                       'OR UIP_BIGO LIKE %s ', [QuotedStr('%'+edFind.Text+'%'), QuotedStr('%'+edFind.Text+'%')]);
...
procedure TfmBlockIp.edFindKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
