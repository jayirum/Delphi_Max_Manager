...
// BusinessSkinForm_1042
  BusinessSkinForm, RzPanel, bsRibbon, bsMessages, bsSkinCtrls,
// Raize, kcRaize
  RzEdit, RzCmboBx, RzDBCmbo, RzSplit, RzDBNav, RzLstBox, RzDBEdit,
...
procedure TfmScL.FormCreate(Sender: TObject);
begin
  inherited;
  SetADOConn(fmScL);
end;
...
begin
  try
    ...
    sSql := Format(
      ...
      '      ,BIGO         ' +
      '  FROM BATCH_SCL    ' +
      ' WHERE SUBSTRING(BS_CD,1,1) IN (%s,%s) ' +
      ' ORDER BY ARTC_CD,MKT_TP ASC ',
      [QuotedStr('S'),
       QuotedStr('E')]);
