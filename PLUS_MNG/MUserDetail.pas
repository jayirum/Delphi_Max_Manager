...
var
  ...
  procedure UserDetail_Run(sId, sTp, sDate : String);
...
//  showmessage(Format('%s , %s , %s, %s', [sId, sTp, sDate, _Trade_DT]));
  ...
  try
    fmUserDetail.ShowModal;
  finally
    fmUserDetail.Free; // TODO : Close 로 수정 후 Action := caFree 추가
  end;
end;

procedure TfmUserDetail.btnCloseClick(Sender: TObject);
begin
  Close;
end;
...
begin
  try
    ...
    sSql := Format(
      ...
      '       LEFT OUTER JOIN (SELECT ISNULL(SUM(NCLR_POS_QTY),0) AS NCLR_POS_QTY, ACNT_NO ' +
      '                          FROM NCLR_POS                                  ' +
      '                         GROUP BY ACNT_NO) D                             ' +
    ...
    with dbMain do begin
      pnTotAcntAmt.Caption := FormatFloat(FORMAT_AMT, FieldByName('ACNT_AMT_TODAY').AsFloat);
      pnTodayPL.Caption    := FormatFloat(FORMAT_AMT, FieldByName('NET_PL_SUM').AsFloat);

      if CompareValue(TextToFloat(pnTodayPL.Caption),0) < 0 then
        pnTodayPL.Font.Color := clBlue
      else
      if CompareValue(TextToFloat(pnTodayPL.Caption),0) > 0 then
        pnTodayPL.Font.Color := clRed
      else
        pnTodayPL.Font.Color := clBlack;

      pnTotPL.Caption := FormatFloat(FORMAT_AMT, FieldByName('TOT_NET_PL_SUM').AsFloat);

      if CompareValue(TextToFloat(pnTotPL.Caption),0) < 0 then
        pnTotPL.Font.Color := clBlue
      else
      if CompareValue(TextToFloat(pnTotPL.Caption),0) > 0 then
        pnTotPL.Font.Color := clRed
      else
        pnTotPL.Font.Color := clBlack;
      ...
      if FieldByName('CONN_YN').AsString = 'Y' then
        pnConn.Caption := '접속중'
      else
        pnConn.Caption := '미접속';
    ...
    sSql := Format(
      'SELECT ISNULL(SUM(RSLT_AMT),0) AS IN_AMT, COUNT(*) AS CNT ' +
      '  FROM INOUT        ' +
      ' WHERE USER_ID = %s ' +
      '   AND IO_TP   = 1  ' +
      '   AND RSLT_TP = 1  ' +
    ...
    pnTotInAmt.Caption := Format('%s (%d)', [FormatFloat(FORMAT_AMT, dbMain.FieldByName('IN_AMT').AsFloat),dbMain.FieldByName('CNT').AsInteger]);

    sSql := Format(
      'SELECT ISNULL(SUM(RSLT_AMT),0) AS OUT_AMT, COUNT(*) AS CNT ' +
      '  FROM INOUT        ' +
      ' WHERE USER_ID = %s ' +
      '   AND IO_TP   = 2  ' +
      '   AND RSLT_TP = 1  ' +
    ...
    fnSqlOpen(dbMain, sSql);
    pnTotOutAmt.Caption := Format('%s (%d)', [FormatFloat(FORMAT_AMT, dbMain.FieldByName('OUT_AMT').AsFloat),dbMain.FieldByName('CNT').AsInteger]);
...
procedure TfmUserDetail.FormCreate(Sender: TObject);
begin
  SetADOConn(fmUserDetail);
end;
...
//  if _sTp = '1' then pnTp.Caption := '국내파생' else
//  if _sTp = '2' then pnTp.Caption := '해외파생';
