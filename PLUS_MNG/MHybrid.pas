  ...
  with DataSet do begin
    if State in [dsEdit] then begin
      ...
      if FieldByName('APITP').AsString <> FieldByName('API_TP').AsString then begin
        if bsMsgYesNo(Format('[%s] -> [%s] 전환 하시겠습니까?', [sTpBNm,sTpANm]), '사용자전환') then begin
...
begin
  ...
  PartTableOpen(rgType,  CodeFormat('ACNT_TP', 'AND CODE_DISP = ''Y'' ORDER BY CODE_VALUE'), '[전체]', '0');
  PartTableOpen(lbxPart, CodeFormat('API_TP', 'AND CODE_VALUE <> ''H'' ORDER BY CODE_VALUE'), '[전체]', '0');
...
begin
  try
    ...
    sSql := Format(
      ...
      '              ,CASE WHEN ( (A.ARTC_CD IN (%s, %s) OR A.ARTC_CD IN (%s, %s) ) AND (SUM(A.AVG_PRC) < 0)) '+
...
begin
  ...
  try
    with dbMain do begin
      ...
      StrToArr(FieldByName('ACNT_TP').AsString,       TC001.TRHeader.ACNT_TP);                   // 1:국내파생, 2:해외파생, 3:국내현물
