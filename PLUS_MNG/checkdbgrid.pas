...
type
  ...
  private
    ...
    FSelectCheckbox: boolean; // i kdk 개별 SelectCheckbox를 그릴지 여부를 제어하기 위해
    ...
    procedure SetSelectCheckbox(const Value: boolean);  // i kdk 속성값을 받아 변수에 넣어 주는 부분
  protected
  ...
  published
    ...
    // i kdk 개별SelectCheckbox를 그릴지 여부를 속성에 넣기 위해
    property SelectCheckbox: boolean read FSelectCheckbox write SetSelectCheckbox default true;
  end;
...
{$ENDIF}
  ...
  uState: UINT;
begin
  if SelectCheckbox then  // i kdk 개별checkbox를 그릴지 여부를 판단
  begin
  {$IFDEF DELPHIXE2_HIGHER}
...
begin
  ...
  begin
    ...
    if DataLink.Active and (SelectedRows.Count = DataLink.DataSource.DataSet.RecordCount) then
     ...
     // c kdk  전체선택에서 하나라도 빠지면 전체선택 체크해제
...
procedure TCheckDBGrid.SetSelectCheckbox(const Value: boolean);
begin
  FSelectCheckbox := Value;
  Invalidate;
end;
...
begin
  ...
  if SelectedRows.Count >= 1 then  // c kdk row가 하나만 있어도 전체해제 되도록
  begin
    SelectedRows.Clear;
  //  SelectedRows.CurrentRowSelected := true; // c kdk 전체선택해제시에는 모두 체크박스를 풀기 위해
  end
  else
    SelectAll;
  DrawTitle0;
