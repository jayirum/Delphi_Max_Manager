...
var
  ...
  sSQL, sGrade: String;
begin
  OffsetS := (_CurrPage * 100) + 1;
  OffsetE := (_CurrPage + 1) * 100;

  // 데이터셋이 열려 있다면 데이터를 추가하도록 설정
  sSQL := fnMainSql;
  ...
  sSQL := StrReplace(sSQL, 'SELECT ', 'WITH ORDERED AS ( SELECT ROW_NUMBER() OVER (ORDER BY A.USER_ID) AS ROW_NUM, ', False);
  sSQL := sSQL + Format(' ) SELECT * FROM ORDERED WHERE ROW_NUM BETWEEN %s AND %s', [IntToStr(OffsetS), IntToStr(OffsetE)]);

  try
    Delay_Show;
    if _CurrPage = 0 then begin
      fnSqlOpen(dbMain, sSQL);
      _TotCnt := dbMain.RecordCount;
    end else begin
      fnSqlDataAdd(dbMain, dbTemp, sSQL);
      _TotCnt := _TotCnt + dbTemp.RecordCount;
    end;
    //ScrollBar1.Max := dbMain.RecordCount;
  finally
    Delay_Hide;
  end;
end;
...
procedure TfmUser.ScrollBar1Scroll(Sender: TObject; ScrollCode: TScrollCode;
  var ScrollPos: Integer);
begin
//  inherited;
  if (ScrollBar1.Position + ScrollBar1.PageSize >= ScrollBar1.Max) and
     (_TotCnt = ScrollBar1.Position) then begin
