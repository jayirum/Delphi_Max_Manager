...
//  edPass.Text := 'ADMIN1228';
  ...
  with dbSQL do begin
    if fnSqlOpen(dbSQL, sSql) = '' then begin
      sCorpNm  := FieldByName('CORP_NM').AsString;
      sCorpTel := FieldByName('CORP_TEL').AsString;
      pnTel.Caption := Format(' %s   Tel.%s', [sCorpNm, sCorpTel]);
      lblUser.Caption := Format('%s %s', [sCorpNm, '(챔피언 관리자)']);
    end;
  end;
...
//  MastDB.iwNotiClient.DataToSend := sPc001 + __EOL;
  ...
  Set_CFGFile('LOGIN', 'IDSAVE', BoolToStrInt(chIDSave.Checked));
